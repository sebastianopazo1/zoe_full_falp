...
def main():
    ...
    if len(images) < 2:
        ...
        return
    stitcher = Stitcher()
    result12 = stitcher.stitch([images[1], images[2]])
    result123 = stitcher.stitch([images[0], result12 ])
    
    if result123 is not None:
        result_cropped = crop_black_borders(result123)
        result_cropped2 = result_cropped[:, :images[0].shape[1]]
        cv2.imwrite("./stitched_zoe/147_4.jpg", result_cropped2)
