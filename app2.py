...
minio_client = Minio(
    ...
    secure=False  # Para HTTPS pasar a True
)
...
def process_images():
    try:
        ...
        output_ply_path = "/tmp/output.ply"
        o3d.io.write_point_cloud(output_ply_path, pcd)

        # Subir a Minio
        minio_output_path = f"{folder_prefix}/output.ply"
        upload_to_minio(output_ply_path, minio_output_path)

        return jsonify({"message": "Archivo subido con exito", "file_url": minio_output_path})
